...
def extract_keywords_to_json(user_prompt: str, vocabulary: dict) -> Dict[str, Any]:
    """
    Passes the prompt to the local Ollama SLM (qwen2.5:0.5b) to extract parameters as JSON.
    Falls back to a default result dict (with empty color_hex/theme_description)
    when the SLM model is not available.
    """
...
"""
    ...
    try:
        ...
        logger.error(f"Failed to connect to Ollama SLM: {e}")
        raise RuntimeError("Ollama is not available") from e
