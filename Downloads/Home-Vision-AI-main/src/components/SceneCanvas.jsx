  ...
  return (
    <>
      {rooms.map(room => {
        ...
        const xPos = (room.x - 12) * SCALE;
        const zPos = (room.z - 10) * SCALE;
  ...
  return (
    <group name="roof-slab" userData={{ hideInBlueprint: true }} position={[0, baseY, 0]}>
      ...
      {isTopFloor && bounds && indianOptions?.flat_terrace && indianOptions?.parapet && (
        <group position={[bounds.x, WALL_HEIGHT + 0.5, bounds.z]}>
          <mesh castShadow position={[0, 0, bounds.length / 2]}>
            <boxGeometry args={[bounds.width, 1, 0.2]} />
          ...
          <mesh castShadow position={[bounds.width / 2, 0, 0]}>
            <boxGeometry args={[0.2, 1, bounds.length]} />
            <meshStandardMaterial color="#cbd5e1" />
          </mesh>
          <mesh castShadow position={[-bounds.width / 2, 0, 0]}>
            <boxGeometry args={[0.2, 1, bounds.length]} />
...
function WalkthroughController() {
  ...
  useEffect(() => {
    if (viewMode === "walk") {
      ...
      if (entranceRoom) {
        const worldX = (entranceRoom.x - 12) * SCALE + offset[0];
        const worldZ = (entranceRoom.z - 10) * SCALE + offset[2];
        const worldW = entranceRoom.width * SCALE;
        const worldL = entranceRoom.length * SCALE;
        console.log("Room raw data → x:", entranceRoom.x, "z:", entranceRoom.z, "w:", entranceRoom.width, "l:", entranceRoom.length);
        console.log("After transform → worldX:", worldX, "worldZ:", worldZ, "worldW:", worldW, "worldL:", worldL);
        spawnX = worldX + worldW / 2;
        spawnZ = worldZ + worldL / 2;
        yaw = Math.PI;
        console.log("Camera spawn → X:", spawnX, "Y:", EYE_LEVEL, "Z:", spawnZ);
      } else {
        console.log("WARNING: No entrance room found, camera at origin (0, 0, 0)");
      }
      ...
      console.log("Camera final position:", camera.position.toArray());
      console.log("Camera rotation (YXZ):", { yaw: mouseState.current.yaw, pitch: mouseState.current.pitch });
      console.log("WALL_HEIGHT:", WALL_HEIGHT, "EYE_LEVEL:", EYE_LEVEL);
      console.log("=== END WALK DEBUG ===");
...
function SceneContent() {
  ...
  const { houseCenter, maxDist } = useMemo(() => {
    ...
    let dist = 18;
    if ((project.floors ? project.floors[project.current_floor_index || 0].rooms : []) && (project.floors ? project.floors[project.current_floor_index || 0].rooms : []).length > 0) {
      const minX = Math.min(...project.rooms.map(r => r.x));
      const maxX = Math.max(...project.rooms.map(r => r.x + r.width));
      const minZ = Math.min(...project.rooms.map(r => r.z));
      const maxZ = Math.max(...project.rooms.map(r => r.z + r.length));
  ...
  const { focusCenter, focusDist, focusMinDist, focusMaxDist } = useMemo(() => {
    ...
    const toFocus = (b, yMid) => {
      const cx = ((b.minX + b.maxX) / 2 - 12) * SCALE + HOUSE_OFFSET[0];
      const cz = ((b.minZ + b.maxZ) / 2 - 10) * SCALE + HOUSE_OFFSET[2];
